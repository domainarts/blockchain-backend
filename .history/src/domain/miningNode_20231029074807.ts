...
export class MiningNode implements IMiningNode {
    ...
    addBlockToBlockchain(block: Block): Block {
        ...
        if (block.transaction.from !== this.blockchain.rewardsTransactionName) this.applyReward(block);
        this.terminateCurrentBlock();
        this.mine();
        console.log(
    ...
    applyReward(block: Block): void {
        if(block.nodeId === this.id) {
            const transaction = this.blockchain.getRewardTransaction(block.transaction.to);
