...
export interface IMiningNode extends IObserver {
    ...
    validateBlock(block: BlockDTO): Promise<any>
}
...
export class MiningNode implements IMiningNode {
    ...
    async validateBlock(block: BlockDTO): Promise<any> {
        const lastBlock = this.blockchain.getLastBlock()
        ...
        if (block.lastHash === lastBlock.currentHash) {
            ...
            if (hash === block.currentHash) {
                ...
                await this.terminateCurrentBlock()
                return block
            }
        }
    ...
    async terminateCurrentBlock(): Promise<void> {
        if (this.currentBlock) {
            return await this.currentBlock.terminateMining()
        }
        //this.isMining = false
    }
