...
export class MiningNode implements IMiningNode {
    ...
    wallet: string
    queue: any
    private static _instance: MiningNode = null;

    private constructor(id: string, name: string, wallet: string) {
        this.id = id
        this.name = name
        ...
        this.wallet = wallet
        this.queue = BullModule.registerQueue({
            name: this.id,
        });
    ...
    static getInstance(id?: string, name?: string, wallet?: string) {
        if (MiningNode._instance === null) {
            MiningNode._instance = new MiningNode(id, name, wallet);
        }
    ...
    async addTransaction(transaction: TransactionDTO): void {
    ...
    validateBlock(block: BlockDTO): any {
        ...
        console.log(
            `MiningNode: [${this.id}] ${this.name} returns block is INVALID.`,
            this.currentBlock.blockId
        )
